      ...
      if (result!=0){
        ...
        peticion.subscribe((result: any) =>  {
            ...
            .subscribe((r:any)=> {
              ...
              .subscribe((resp: any) =>  {
                let us = resp;  
                this.cuerpoCorreo = '<html><head><style>table{font-family: arial, sans-serif;border-collapse:collapse;width: 100%;} td,th{border: 1px solid #dddddd; text-align: left;  padding: 8px;} tr{background-color:#D1ECF1;} tr:nth-child(even) {background-color:#FFFFFF;}</style></head><body><h3>Su reserva se ha registrado exitosamente segun el siguiente detalle:</h3><br><table><tr><th>Servicio</th><th>Fecha</th>    <th>Hora</th>    <th>Terapista</th>  </tr><tr><td>' + servicio +'</td><td>' + fecha +'</td><td>' + hora +'</td><td>' + terapista +'</td></tr></table></body><br> <br><h3>Puedes explorar más servicios escanenado el código QR.</h3><br><div id="qrcode"><img src="https://www.codigos-qr.com/qr/php/qr_img.php?d=https%3A%2F%2Fspa-2021-angular.herokuapp.com%2Fbooking%2Fcategorias&s=6&e=m" alt="Generador de Códigos QR Codes"/><br/><a href="https://www.codigos-qr.com/en/qr-code-generator/" target="_blank" id"qrgenerator">...</a></div><br><div>Katthy Spa S.A.<br>10 DE AGOSTO Y GRAL. CABALLERO<br>TELEFONO 021-498-690<br>SAN LORENZO - PARAGUAY</div></html>';
                if (this.admin !== true ){
